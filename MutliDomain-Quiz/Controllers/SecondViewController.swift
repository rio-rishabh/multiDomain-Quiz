...
class SecondViewController: UIViewController {
    ...
    var quizBrain_2 = QuizBrain_2()
    
    @IBAction func answerButtonPressed(_ sender: UIButton) {
        let userAnswer = sender.currentTitle!
        
        let userCorrectAnswer = quizBrain_2.checkAnswer(userAnswer)
        ...
        quizBrain_2.getNewQuestion()
        
        Timer.scheduledTimer(timeInterval: 0.5, target: self, selector: #selector(updateUI), userInfo: nil, repeats: false)
        
    }
    @objc func updateUI(){
        questionLabel.text = quizBrain_2.getQuestionText()
        
        let answerChoice = quizBrain_2.getAnswerChoice()
        scoreLabel.text = "Score: \(quizBrain_2.getScore())"
